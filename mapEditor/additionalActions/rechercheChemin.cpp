...
{
    portee(cases_pleines, cases_ateignables, xdep, ydep, true,true,true,true,true,true,true,true, max_portee);
    for(int i = 0; i < NBR_CASES_L; i++)
    {
        for(int j = 0; j < NBR_CASES_H; j++)
        {

            cases_ateignables[i][j] = cases_ateignables[i][j] && (cases_pleines[i] [j]==0);
        }
    }
}
